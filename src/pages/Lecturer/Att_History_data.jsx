  ...
  const handleOpenPDF = (event) => {
    ...
    if (selectedAcademicYear == "Select a Academic Year") {
      ...
      localStorage.setItem(
        "attendanceData",
        JSON.stringify(allstudentsattendace),
      );
      localStorage.setItem(
        "subject_attendance",
    ...
    const fetchattendance = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/lecturer/get_history_attendace/${idpassed}/${subjectpassed}/${yearpassed}`,
        );
  ...
  const FeedbackModal = ({ onClose, feedbacktext }) => {
    return (
      ...
      >
        ...
        >
          <AiOutlineClose
            className="absolute   rounded-xl p-1 right-6 hover: bg-slate-400 top-6 text-3xl text-yellow-50 cursor-pointer"
            onClick={onClose}
          />
          <h2 className="text-lg  mt-5 font-semibold ml-4">
            Feedback of student
          </h2>
        ...
        <div className="grid grid-cols-1 gap-5 p-6 md:grid-cols-3 ">
          <div className="flex flex-col md:col-span-2 w-full">
            <div className="p-3 font-medium text-center bg-white rounded shadow sm:text-1xl lg:text-lg rounded-2xl w-full">
              <h3 className="px-4 mt-2 mb-4 text-lg font-semibold text-center">
                Attendance Data of Subject
              </h3>
                      ...
                      allattendace.length > 0 ? (
                        allattendace.map((item, index) => {
                          ...
                          return (
                            ...
                            >
                              <td
                                className={`p-4 text-[12px] font-medium break-words whitespace-normal leading-tight	 max-w-[180px] overflow-hidden ${
                              ...
                              <td
                                className={`p-4  text-[12px] font-medium break-words max-w-[180px] ${
                                  isSelected ? "text-white" : "text-slate-900"
                                }`}
                              >
                                {item[8]}
                        ...
                        <tr>
                          <td
                            colSpan={4}
                            className="p-4 text-[10px] text-center text-gray-500"
                          >
          ...
          <div className="flex flex-col w-full h-full justify-content-center items-center">
            <div className="flex flex-col max-w-full  mx-0 overflow-hidden bg-white shadow-lg sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl md:ml-4 rounded-2xl p-5">
              <h3 className="px-4 text-lg font-semibold text-center">
                Students Data{" "}
              </h3>
              <div className="h-50 sm:h-60 md:h-80 lg:h-flex xl:h-[330px] overflow-auto px-2 pt-3">
                <table className="max-w-full text-[12px] bg-white">
                  ...
                  {Array.isArray(allstudentsattendace) &&
                  allstudentsattendace.length > 0 ? (
                    <tbody className="whitespace-nowrap">
